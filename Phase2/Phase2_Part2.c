...
#define max_mes 10
#define phone_len 12
#define phone_address "phone.txt"
#define user_address "user.txt"
#define pass_address "pass.txt"
#define sent_mes_address "sent messages.txt"
#define recieved_mes_address "recieved messages.txt"
//====================================================================================================================
// declaring functions
...
void read_data(char phone[][phone_len], char user[][max_len], char pass[][max_len], char sent_messages[n_users][n_users][max_mes][mes_max_len], char recieved_messages[n_users][n_users][max_mes][mes_max_len]);
void write_data(char phone[][phone_len], char user[][max_len], char pass[][max_len], char sent_messages[n_users][n_users][max_mes][mes_max_len], char recieved_messages[n_users][n_users][max_mes][mes_max_len]);
//====================================================================================================================
// defining functions
void main_menu(int p_func_check, char phone[][phone_len], char user[][max_len], char pass[][max_len], char sent_messages[n_users][n_users][max_mes][mes_max_len], char recieved_messages[n_users][n_users][max_mes][mes_max_len])
{
	...
	if (mm_comm_check == 1)
	{
		...
		else if (mm_comm == 3) /*Quit*/
		{
			...
			write_data(phone, user, pass, sent_messages, recieved_messages);
			return;
		}
...
{
	int check;
	FILE *fphone = fopen(phone_address, "r");
	if (fphone != NULL)
	{
	...
	FILE *fuser = fopen(user_address, "r");
	if (fuser != NULL)
	{
		char temp[max_len + 1];
		int i = 0;
	...
	FILE *fpass = fopen(pass_address, "r");
	if (fpass != NULL)
	{
		char temp[max_len + 1];
		int i = 0;
	...
	FILE *fsent_messages = fopen(sent_mes_address, "r");
	if (fsent_messages != NULL)
	{
		char ch;
		for (int i = 0; i < n_users; i++)
		{
			for (int j = 0; j < n_users; j++)
			{
				for (int k = 0; k < max_mes; k++)
				{
					if (k == 0)
					{
						ch = fgetc(fsent_messages);
						if (ch == '\n')
							break;
						else
							ungetc(ch, fsent_messages);
					}
					fscanf(fsent_messages, "%[^;]s", sent_messages[i][j][k]);
					ch = fgetc(fsent_messages); //this is ; character
					ch = fgetc(fsent_messages);	// can be next string first letter or enter
					if (ch == '\n')
						break;
					else
						ungetc(ch, fsent_messages);
		...
		fclose(fsent_messages);
	}
	FILE *frecieved_messages = fopen(recieved_mes_address, "r");
	if (frecieved_messages != NULL)
	{
		char ch;
		for (int i = 0; i < n_users; i++)
		{
			for (int j = 0; j < n_users; j++)
			{
				for (int k = 0; k < max_mes; k++)
				{
					if (k == 0)
					{
						ch = fgetc(frecieved_messages);
						if (ch == '\n')
							break;
						else
							ungetc(ch, frecieved_messages);
					}
					fscanf(frecieved_messages, "%[^;]s", recieved_messages[i][j][k]);
					ch = fgetc(frecieved_messages);
					ch = fgetc(frecieved_messages);
					if (ch == '\n')
						break;
...
{
	char null[] = { NULL };
	FILE* fphone = fopen(phone_address, "w");
	for (int i = 0; i < n_users; i++)
	{
		if (strcmp(phone[i], null) == 0)
			break;
	...
	FILE* fuser = fopen(user_address, "w");
	for (int i = 0; i < n_users; i++)
	{
		if (strcmp(user[i], null) == 0)
			break;
		else
			fprintf(fuser, "%s\n", user[i]);
	}
	FILE* fpass = fopen(pass_address, "w");
	for (int i = 0; i < n_users; i++)
	{
		if (strcmp(pass[i], null) == 0)
			break;
		else
			fprintf(fpass, "%s\n", pass[i]);
	}
	FILE* fsent_messages = fopen(sent_mes_address, "w");
	for (int i = 0; i < n_users; i++)
	{
		for (int j = 0; j < n_users; j++)
		{
			for (int k = 0; k < max_mes; k++)
			{
				if (strcmp(sent_messages[i][j][k], null) != 0)
					fprintf(fsent_messages, "%s;", sent_messages[i][j][k]);
				else
					break;
			}
			fprintf(fsent_messages, "\n");
		}
	}
	FILE* frecieved_messages = fopen(recieved_mes_address, "w");
	for (int i = 0; i < n_users; i++)
	{
		for (int j = 0; j < n_users; j++)
		{
			for (int k = 0; k < max_mes; k++)
			{
				if (strcmp(recieved_messages[i][j][k], null) != 0)
					fprintf(frecieved_messages, "%s;", recieved_messages[i][j][k]);
				else
					break;
			}
			fprintf(frecieved_messages, "\n");
		}
	}
	return;
}
...
int main(void)
{
	...
	//------------------------------------------------------------------------------
	read_data(phone, user, pass, sent_messages, recieved_messages);
	//------------------------------------------------------------------------------
	printf("welcome!\n\n");
	main_menu(0, phone, user, pass, sent_messages, recieved_messages);
	return 0;
